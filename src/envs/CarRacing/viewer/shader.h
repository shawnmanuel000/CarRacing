...
class Shader
{
	private:
		...
		std::string loadShaderSource(std::string fileName)
		{
			...
			if (!in_file.is_open())
			{
				std::string error = "ERROR::SHADER::COULD_NOT_OPEN_FILE: " + fileName;
	...
	public:
		Shader(const int versionMajor, const int versionMinor, std::string vertexFile, std::string fragmentFile) : versionMajor(versionMajor), versionMinor(versionMinor)
		{
